...
/**
 * /connect: the URL a headless CLI prints. Dispatches to the active cloud
 * auth backend — each variant waits for its own session, then forwards the
 * CLI's PKCE request to that backend's authorize endpoint.
 */
export function ConnectCliAuthorizeSurface() {
  if (cloudAuthMode() === "oidc") return <OidcConnectCliAuthorizeSurface />;
  return <ClerkConnectCliAuthorizeSurface />;
}
...
function OidcConnectCliAuthorizeSurface() {
  const [request] = useState(() => readConnectAuthorizeRequest(new URL(window.location.href)));
  const cloudAuth = useCloudAuth();
  const redirecting = useRef(false);

  useEffect(() => {
    if (!request || !cloudAuth.isLoaded || redirecting.current) {
      return;
    }
    ...
    redirecting.current = true;
    void (async () => {
      const authorizeUrl = await buildConnectCliOidcAuthorizeUrl(request);
      if (!authorizeUrl) {
  ...
  return (
    <AuthSurfaceShell>
      ...
      />
      {cloudAuth.isLoaded && !cloudAuth.isSignedIn ? (
        <div className="mt-6">
          <Button type="button" onClick={() => void cloudAuth.signIn(window.location.href)}>
            Sign in
          </Button>
        </div>
      ) : null}
...
/**
 * /connect/callback: the active backend's redirect target. Shows the
 * one-time code the user enters in the waiting terminal.
 */
export function ConnectCliCallbackSurface() {
  if (cloudAuthMode() === "oidc") return <OidcConnectCliCallbackSurface />;
  return <ClerkConnectCliCallbackSurface />;
}
...
function OidcConnectCliCallbackSurface() {
  const [result] = useState(readConnectCliCallbackResult);
  const [expectedState] = useState(readConnectCliAuthState);
  const cloudAuth = useCloudAuth();
  const { copyToClipboard, isCopied } = useCopyToClipboard({ target: "authentication code" });
  ...
  const accountLabel = cloudAuth.displayIdentity;
  const authCode = encodeConnectAuthCode(result);
