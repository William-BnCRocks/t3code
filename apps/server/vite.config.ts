  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        ...
        __T3CODE_BUILD_OIDC_ISSUER_URL__: JSON.stringify(
          repoEnv.T3CODE_OIDC_ISSUER_URL?.trim() ?? "",
        ),
        __T3CODE_BUILD_OIDC_CLI_CLIENT_ID__: JSON.stringify(
          repoEnv.T3CODE_OIDC_CLI_CLIENT_ID?.trim() ?? "",
        ),
