...
import {
  ...
  ListUserInputProjectionThreadActivitiesInput,
  ProjectionThreadActivity,
  ProjectionThreadActivityRepository,
  type ProjectionThreadActivityRepositoryShape,
  USER_INPUT_PROJECTION_ACTIVITY_KINDS,
...
const makeProjectionThreadActivityRepository = Effect.gen(function* () {
  ...
  // Same SELECT shape and ORDER BY as `listProjectionThreadActivityRows`,
  // narrowed to the kinds `derivePendingUserInputCountFromActivities` reads.
  const listUserInputProjectionThreadActivityRows = SqlSchema.findAll({
    Request: ListUserInputProjectionThreadActivitiesInput,
    Result: ProjectionThreadActivityDbRowSchema,
    execute: ({ threadId }) =>
      sql`
        SELECT
        ...
        FROM projection_thread_activities
        WHERE thread_id = ${threadId}
          AND kind IN ${sql.in(USER_INPUT_PROJECTION_ACTIVITY_KINDS)}
        ORDER BY
          CASE WHEN sequence IS NULL THEN 0 ELSE 1 END ASC,
          sequence ASC,
          created_at ASC,
  ...
  const listUserInputActivitiesByThreadId: ProjectionThreadActivityRepositoryShape["listUserInputActivitiesByThreadId"] =
    (input) =>
      listUserInputProjectionThreadActivityRows(input).pipe(
        Effect.mapError(
          toPersistenceSqlOrDecodeError(
